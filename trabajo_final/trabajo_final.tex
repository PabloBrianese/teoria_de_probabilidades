...
Si \(t \geq h\), entonces sabemos que \(A \cap E_{\pre{t}}\) es \(\borel(\interval{cc}{0}{\pre{t}}) \otimes \events(\pre{t})\)--medible, donde \(\pre{t} = t - h\). 
\begin{align}
  A' \cap E_t
  &=
...
Además, todos los elementos de su imágen están contenidos en \(\pro{\left( \interval{cc}{0}{\pre{t}} \times \Omega \right)} = \interval{cc}{h}{t} \times \Omega\), y este conjunto sí es claramente \(\borel(\interval{cc}{0}{t}) \otimes \events(t)\)--medible.
...
\begin{itemize}
  ...
  \item \(\pro{\left( \bigcup_{i \in I} U_i \right)} =  \bigcap_{i \in I} \pro{U_i}\)
  \item \(\pro{\left( E_{\pre{t}} \setminus U \right)} = \image{\sharp} \setminus \pro{U}\)
\end{itemize}
