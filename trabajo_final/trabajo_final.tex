...
\begin{example}
  ...
  Dado un intervalo de tiempo \(\interval{oc}{t^{\flat}}{t^{\sharp}}\) y una variable \(\events(t^{\sharp})\)--medible \(A\), la función 
  \begin{align}
    &X(t, \omega) = A(\omega) \characteristic\left( \interval{oc}{t^{\flat}}{t^{\sharp}} \right)(t)
    && (\forall (t, \omega) \in \interval{co}{0}{\infty} \times \Omega)
  \end{align}
  da lugar a un proceso progresivamente medible.
  Para probarlo consideremos un instante \(t^* \geq 0\).
  \newcommand{\restricted}[1]{\overline{#1}}
  La restricción \(\restricted{X}\) de \(X\) al conjunto \(\interval{cc}{0}{t^*} \times \Omega\) es
  \begin{align}
    &\restricted{X}(t, \omega)
    =
    \left\{\begin{aligned}
      &0
        &&\text{si } t^* \leq t^{\flat} 
      \\
      &A(\omega) \characteristic\left( \interval{oc}{t^{\flat}}{t^{\sharp} \wedge t^*} \right) (t) 
        &&\text{si } t^* > t^{\flat}
    \end{aligned}\right.
    && (\forall (t, \omega) \in \interval{cc}{0}{t^*} \times \Omega)
  \end{align}
  En el primer caso resulta constante, y \(\borel(\interval{cc}{0}{t^*}) \otimes \events(t^*)\)--medible de forma inmediata.
  Sobre el segundo caso debemos detenernos.
  Para aquellos borelianos \(B \in \borel(\realNumbers)\) con \(0 \notin B\), podemos calcular que para todo \((t, \omega) \in \interval{cc}{0}{t^*} \times \Omega\)
  \begin{align}
    X(t, \omega) \in B
    &\leftrightarrow
      A(\omega) \characteristic\left( \interval{oc}{t^{\flat}}{t^{\sharp} \wedge t^*} \right) (t) \in B
    \\
    &\leftrightarrow
      A(\omega) \in B,\: t^{\flat} < t \leq t^{\sharp} \wedge t^*
    \\
    &\leftrightarrow
      (t, \omega) \in \interval{oc}{t^{\flat}}{t^{\sharp} \wedge t^*} \times [A \in B]
  \end{align}
...
\begin{proof}
  ...
  Si \(t' \geq t\), \(\left. X_n \right|_{[0, t'] \times \Omega} = X_n\).
  Para todo \(B \in \borel(\realNumbers)\); si \(0 \notin B\), \(X_n(s, \omega) \in B\) si y solo si \((s, \omega) \in \bigcup_{k = 0}^{2^n - 1} \interval{oc}{t_k}{t_{k + 1}} \times [X_n(t_{k + 1}) \in B]\)
