...
La aplicación \((t, \omega) \mapsto ((t - h) \vee 0, \omega)\) es \(\progressiveAlgebra\)--medible?

Sea \(A \in \progressiveAlgebra\) y un desplazamiento \(h > 0\).
\newcommand{\pro}[1]{{#1}^{\sharp}}
\newcommand{\pre}[1]{{#1}^{\flat}}
Trasladamos \(A\) escribiendo \(\pro{A} = \{(\pro{s}, \omega) : (s, \omega) \in A\}\) donde \(\pro{s} = s + h\).
Tenemos que probar \(\pro{A} \in \progressiveAlgebra\).
Fijemos \(t \geq 0\) arbitrario.
Queremos demostrar que \(\pro{A} \cap E_t\) también.
Si \(t < h\), entonces \(\pro{A} \cap E_t = \emptyset\) es medible en cualquier caso.

Si \(t = h\), .....

Si \(t > h\), entonces sabemos que \(A \cap E_{\pre{t}}\) es \(\borel(\interval{cc}{0}{\pre{t}}) \otimes \events(\pre{t})\)--medible, donde \(\pre{t} = t - h\). 
\begin{align}
  A' \cap E_t
  &=
  ...
  \\
  &=
  \{(\pre{s} + h, \omega) \in E_t : (\pre{s}, \omega) \in A\}
  \\
  &=
  \pro{\{(r, \omega) \in E_{t - h} : (r, \omega) \in A\}}
\end{align}
Lo que tenemos que probar es que 
\begin{align}
  \borel\left(\interval{cc}{0}{\pre{t}}\right) \otimes \events(\pre{t}) 
  \xhookrightarrow{\quad \sharp \quad}
  \borel(\interval{cc}{0}{t}) \otimes \events(t)
\end{align}
En un principio, los elementos de la imágen de la aplicación \(\sharp\) son sólo subconjuntos de \(\interval{cc}{0}{t} \times \Omega\).
Sin embargo, de entrada podemos afirmar que es una inyección.
Además, todos los elementos de su imágen están contenidos en \(\pro{E_{\pre{t}}} = \pro{\left( \interval{cc}{0}{\pre{t}} \times \Omega \right)} = \interval{cc}{h}{t} \times \Omega\), y este conjunto sí es claramente \(\borel(\interval{cc}{0}{t}) \otimes \events(t)\)--medible.
Muy importante, esta función respeta la estructura de \(\sigma\)--álgebra que tiene \(\borel\left(\interval{cc}{0}{\pre{t}}\right) \otimes \events(\pre{t})\).

\begin{itemize}
  \item \(\pro{\emptyset} = \emptyset\) Porque si existiera un par \((\pro{s}, \omega) \in \pro{\emptyset}\), entonces existiría otro \((s, \omega) \in \emptyset\) y llegamos a un absurdo.
  \item \(\pro{\left( \bigcup_{i \in I} U_i \right)} =  \bigcup_{i \in I} \pro{U_i}\) Porque si \((\pro{s}, \omega) \in \pro{\left( \bigcup_{i \in I} U_i \right)}\)
  \item \(\pro{\left( E_{\pre{t}} \setminus U \right)} = \pro{E_{\pre{t}}} \setminus \pro{U}\)
\end{itemize}
...
De forma similar, cuando tenemos un par \((s, \omega) \in \interval{cc}{h}{t} \times \Omega\) definimos \(\pre{(s, \omega)} \coloneqq (\pre{s}, \omega)\).
...
Esto es importante porque hace que la función de conjuntos \(\sharp : \parts \left( E_{\pre{t}} \right) \rightarrow \parts \left( \pro{E_{\pre{t}}} \right)\) respete toda operación entre conjuntos.
Es decir que respeta
\begin{enumerate*}
  \item al conjunto vacío, \(\pro{\emptyset} = \emptyset\);
  \item las uniones arbitrarias de conjuntos, \(\pro{(\bigcup_{i \in I} V_i)} = \bigcup_{i \in I} \pro{V_i}\) para toda familia de conjuntos \(V_i \subseteq E_{\pre{t}}\) indexada por \(i \in I\);
