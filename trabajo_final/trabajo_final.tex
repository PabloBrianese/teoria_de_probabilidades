...
Usando el correspondencia estático-dinámica de los procesos llegamos a la siguiente noción. 
\begin{definition}
  ...
  Escribimos \(E = \interval{co}{0}{\infty} \times \Omega\), y usamos \(\mathscr{E}\) para denotar al conjunto de partes progresivamente medibles de \(E\).
\end{definition}
...
\end{theorem}
\begin{proof}
  Las funciones características de los conjuntos \(\emptyset\), \(E\) son \({\characteristic_{\emptyset} = 0}\), \({\characteristic_E = 1}\).
  Luego \(\emptyset, E \in \mathscr{E}\), porque todo proceso constante es progresivamente medible.

  Si \(A \in \mathscr{E}\) es un conjunto progresivamente medible ¿entonces \(E \setminus A \in \mathscr{E}\)? 

  Si \(\{A_n \subseteq E : n \in \naturalNumbers\}\) es una sucesión de procesos progresivamente medibles, entonces ¿\(\bigcap_n A_n \in \mathscr{E}\)?
\end{proof}
