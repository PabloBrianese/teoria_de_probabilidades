...
Hemos notado que un proceso puede ser considerado una función definida en el producto cartesiano \(\left[ 0, \infty \right[ \times \Omega\).
Ahora hacemos uso de esta observación de forma sistemática.

De ese modo, dada una función \(X\) definida en \(\left[ 0, \infty \right[ \times \Omega\) con valores en un espacio medible \((E, \mathscr{E})\), denotamos por \(X(t)\) la aplicación parcial \(\omega \mapsto X(t, \omega)\), e \emph{identificamos} la función \(X\) con la familia de aplicaciones parciales \(\{X(t) : t \geq 0\}\).
Esto permite decir, por ejemplo, que \emph{\(X\) es un proceso} si la familia \(\{X_t\}\) lo es.
También decimos que un subconjunto \(A\) de \(\left[ 0, \infty \right[ \times \Omega\) es \emph{progresivamente medible} con respecto a la familia \((\events_t)\) si su función indicadora queda identificada con un proceso progresivamente medible con respectoa esta familia.
...
Denotamos por \((\Omega, \mathcal{A}, \prob)\) al espacio de probabilidad en que nuestro movimiento Browniano \(\{\brownian(t) : t \geq 0\}\) está definido y supongamos que \((\events(t) : t \geq 0)\) es una filtración a la que el movimiento Browniano está adaptado tal que la propiedad fuerte de Markov se verifica.
