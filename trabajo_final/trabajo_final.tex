...
Un proceso estocástico \((X(t) : t \geq 0)\) adaptado a una filtración \((\events_t : t \geq 0)\) en un espacio de probabilidad \((\Omega, \events, \prob)\) puede ser considerado como una función \(X\) dada por \(X(t, \omega) = X(t)(\omega)\) definida en el producto cartesiano \(\left[ 0, \infty \right[ \times \Omega\).

Recíprocamente, dada una función \(X\) definida en \(\interval{co}{0}{\infty} \times \Omega\) con valores en un espacio medible \((E, \mathscr{E})\), denotamos por \(X(t)\) la aplicación parcial \(\omega \mapsto X(t, \omega)\), e \emph{identificamos} la función \(X\) con la familia de aplicaciones parciales \(\{X(t) : t \geq 0\}\).%
...
Haremos un uso sistemático de esta observación.
