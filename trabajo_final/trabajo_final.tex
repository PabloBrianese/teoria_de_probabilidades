...
\begin{definition}
  Un proceso estocástico \(\{X(t) : t \geq 0\}\) en un espacio de probabilidad \((\Omega, \events, \prob)\) filtrado por \((\events(t) : t \geq 0)\) es llamado \emph{progresivamente medible} si para todo \(t \geq 0\) la aplicación restringida \(X : [0, t] \times \Omega \rightarrow \realNumbers\) es medible con respecto a la \(\sigma\)--álgebra \(\borel([0, t]) \otimes \events(t)\).
\end{definition}

% ISSUE
\begin{lemma}
  La condición de ser progresivamente medible hace que el proceso sea adaptado.
\end{lemma}
...
\begin{proof}
  ...
  Luego \(E \in \progressiveAlgebra\), porque como ya observamos todo proceso constante es progresivamente medible.
