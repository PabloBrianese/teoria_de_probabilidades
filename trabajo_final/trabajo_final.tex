...
\begin{proof}
  ...
  Si escribimos \(U \coloneqq \restrict{X}{E_n}^{- 1}(B)\), esta igualdad se lee 
  \begin{align}
    Y_n = \{t \in \interval{cc}{0}{n} : (t, \omega) \in U\}
  \end{align}
  En palabras, la ecuación dice que \(Y_n \subseteq \interval{cc}{0}{n}\) es una \(\interval{cc}{0}{n}\)--sección en \(\omega \in \Omega\) del conjunto \(U \subseteq E_n\).
  Pero \(U\) es \(\borel(\interval{cc}{0}{n}) \otimes \events(n)\)--medible porque \(X\) es progresivamente medible.
...
\newpage
Traslaciones de procesos progresivamente medibles.
...
\begin{proof}
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Sea \(A \in \progressiveAlgebra\).
  Consideramos su desplazamiento en una cantidad \(h > 0\)
  \begin{align}
    \pro{A} = \{(s + h, \omega) : (s, \omega) \in A\}
  \end{align}
  ...
  Debemos detenernos con cuidado en el caso no trivial \(t \geq h\) para poder hacer consideraciones globales.
  
  Se ruega paciencia, pues en la presente demostración haremos una seria sobrecarga de los símbolos \(\sharp\) y \(\flat\).
  %%%%%%%%%%%%%%%%%%%%%%%%%
  % AGREGAR LA CUESTION 
  % CON LOS INTERVALOS
  %%%%%%%%%%%%%%%%%%%%%%%%%
  Cuando tratamos escalares \(s \in \interval{cc}{h}{t}\), definimos \(\pre{s} \coloneqq s - h\), y cuando \(s \in \interval{cc}{0}{\pre{t}}\) definimos \(\pro{s} \coloneqq s + h\).
  Lo mismo haremos con conjuntos de escalares, si \(J \subseteq \interval{cc}{h}{t}\) definimos \(\pre{J} \coloneqq \{\pre{j} : J \in U\}\), y cuando \(I \subseteq \interval{cc}{0}{\pre{t}}\) definimos \(\pro{I} \coloneqq \{\pro{i} : i \in I\}\).
  De forma similar, cuando tenemos un par \((s, \omega) \in \interval{cc}{h}{t} \times \Omega\) definimos \(\pre{(s, \omega)} \coloneqq (\pre{s}, \omega)\), y si \((s, \omega) \in \interval{cc}{0}{\pre{t}} \times \Omega\) definimos \(\pro{(s, \omega)} \coloneqq (\pro{s}, \omega)\).
  Al tratar conjuntos haremos lo análogo, si \(U \subseteq \interval{cc}{h}{t} \times \Omega\) definimos \(\pre{U} \coloneqq \{\pre{u} : u \in U\}\), y cuando \(V \subseteq \interval{cc}{0}{\pre{t}} \times \Omega\), definimos \(\pro{V} \coloneqq \{\pro{v} : v \in V\}\).
  Notar que \(\interval{cc}{0}{\pre{t}} \times \Omega = E_{\pre{t}}\) y \(\interval{cc}{h}{t} \times \Omega = \pro{E_{\pre{t}}}\).
  
  ¿Por qué es necesario todo esto?
  Pues porque nos permite llegar al significado último del simple cálculo inicial
  \begin{align}
    \pro{A} \cap E_t
    &=
    ...
    \pro{(A \cap E_{\pre{t}})}
  \end{align}
  Que sin embargo encontraremos mediante un razonamiento abstracto.
  Comenzamos observando que \(\sharp : \interval{cc}{0}{\pre{t}} \rightarrow \interval{cc}{h}{t}\)  y, en consecuencia, \(\sharp : E_{\pre{t}} \rightarrow \pro{E_{\pre{t}}}\) son biyecciones.
  Esto es importante porque hace que la función de conjuntos \(\sharp : \parts ( E_{\pre{t}} ) \rightarrow \parts ( \pro{E_{\pre{t}}} )\) se comporta bien con las operaciones entre conjuntos.
  Es decir que respeta
  \begin{enumerate*}
    \item al conjunto vacío, \(\pro{\emptyset} = \emptyset\);
    \item las uniones arbitrarias de conjuntos, \(\pro{\bigcup\{V : V \in \mathcal{V}\}} = \bigcup \{ \pro{V} : V \in \mathcal{V}\}\) para toda familia \(\mathcal{V}\) de conjuntos \(V \subseteq E_{\pre{t}}\);
    \item los complementos, \(\pro{(E_{\pre{t}} \setminus V)} = \pro{E_{\pre{t}}} \setminus \pro{V}\) para todo subconjunto \(V \subseteq E_{\pre{t}}\).
  \end{enumerate*}
  Estos límites estructurales hacen que podamos demostrar que la función de conjuntos \(\sharp : \parts(E_{\pre{t}}) \rightarrow \parts(\pro{E_{\pre{t}}})\) da lugar a una restricción \(\sharp : \borel(\interval{cc}{0}{\pre{t}}) \otimes \events(\pre{t}) \rightarrow \borel(\interval{cc}{h}{t}) \otimes \events(t)\) solamente verificando la relación \(\pro{P} \in \borel(\interval{cc}{h}{t}) \otimes \events(t)\) sobre unos \(P \in \borel(\interval{cc}{0}{\pre{t}}) \otimes \events(\pre{t})\) que generen la \(\sigma\)--álgebra completa.
  La familia de conjuntos \(P\) que elegimos es la de aquellos de la forma \(P = I \times F\) donde \(I \subseteq \interval{cc}{0}{\pre{t}}\) es un intervalo y \(F \in \events(\pre{t})\).
  En efecto, si \(P\) es rectangular de esta manera, se dá la identidad \(\pro{P} = \pro{I} \times F\).
  Aquí \(\pro{I} \subseteq \interval{cc}{h}{t}\) vuelve a ser un intervalo, y \(F\) resulta \(\events(t)\)--medible porque \(\events(\pre{t}) \subseteq \events(t)\), de modo tal que \(\pro{P} \in \borel(\interval{cc}{h}{t}) \otimes \events(t)\).
  Es valioso reflexionar sobre este punto para darse cuenta de por qué la anticipación de un conjunto progresivamente medible no vuelve a ser uno, pero si su retrazo.
  En fin, el resultado concreto de este razonamiento abstracto es que \(\pro{(A \cap E_{\pre{t}})} = \pro{A} \cap E_t\) es un conjunto \(\borel(\interval{cc}{0}{t}) \otimes \events(t)\)--medible porque \(A \cap E_{\pre{t}}\) es \(\borel(\interval{cc}{0}{\pre{t}}) \otimes \events(\pre{t})\)--medible, y esto último es así por la hipótesis \(A \in \progressiveAlgebra\).
  ...
  \begin{proof}
    ...
    Si 0 no es uno de sus elementos, entonces 
    \begin{align}
      S_h X ^{- 1} (B)
      &=
      \{
      ...
      \}
    \end{align}
    Siguiendo la notación de \(\sharp\) y \(\flat\), podemos escribir
    \begin{align}
      S_h X ^{- 1} (B)
      &=
      \{
      ...
      \pro{
        \{
          a \in \interval{co}{0}{\infty} \times \Omega 
          :
          X (a) \in B
        \}
      }
    ...
    Por otra parte, si 0 pertenece a \(B\)
...
\newpage
Acumulaciones de procesos progresivamente medibles.

Hemos probado (en \ref{theorem:instacesOfProgressivelyMeasurableProcessesAreBorelFunctions}) que las trayectorias de un proceso progresivamente medible son funciones borel.
