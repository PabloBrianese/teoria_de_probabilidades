...
\begin{theorem}
  Los desplazamientos de conjuntos progresivamente medibles son conjuntos progresivamente medibles. 
\end{theorem}
\begin{proof}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  Si \(t = h\), podemos calcular la intersección 
  \begin{align}
    \pro{A} \cap E_h 
    &=
    \{(s + h, \omega) : (s, \omega) \in A\} \cap \interval{cc}{0}{h} \times \Omega
    \\
    &=
    \{(h, \omega) : (0, \omega) \in A\}
  \end{align}
  consiste solamente de aquellos pares
...
\begin{align}
  ...
  \\
  &=
  \pro{\{(r, \omega) \in E_{\pre{t}} : (r, \omega) \in A\}}
\end{align}
...
Esto es importante porque hace que la función de conjuntos \(\sharp : \parts ( E_{\pre{t}} ) \rightarrow \parts ( \pro{E_{\pre{t}}} )\) respete toda operación entre conjuntos.
