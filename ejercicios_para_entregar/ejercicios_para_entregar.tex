...
\begin{proof} a)
\((X_t)_{t \geq 0}\) es un proceso de Markov a tiempo continuo, con espacio de estados numerable \(I = \naturalnum = \{0, 1, 2, \dots\}\).
Sea \(T_i = \inf\{t > J_1 : X_t = i\}\) para cada \(i \in I\).
Un estado \(i \in I\) será recurrente si \(\prob^i(T_i<\infty)=1\).
Será recurrente positivo si además \(\Exp^i T_i < \infty\).

% Nota 25 nov. 2020 teoría de proba  clase práctica 20.pdf
Vamos a ver que \(\Exp T_0 < \infty \Leftrightarrow \lambda < \mu\).
...
\begin{enumerate}
	...
	\item Si \(\{X_i\}_{i \in \naturalnum}\) son v.a. independientes con distribuciones \(X_i \sim \Exponential(\lambda_i)\) tales que \(\sum_{i \in \naturalnum} \lambda_i\), entonces
	\begin{itemize}
		\item \(\min_{i \in \naturalnum} X_i \sim \Exponential(\lambda)\) con \(\lambda = \sum_{i \in \naturalnum} \lambda_i\)
		\item \(\prob(\argmin_{i \in \naturalnum} X_i = j) = \lambda_j / \lambda\), con \(\lambda = \sum_{i \in \naturalnum} \lambda_i\), para todo \(j \in \naturalnum\).
	\end{itemize}
\end{enumerate}

Usando estos lemas, podemos pensar que estando la cadena en el estado \(i\), su cambio responde a un único reloj exponencial con tasa \(\lambda + \mu\), y al sonar este reloj se tira una moneda cargada:
