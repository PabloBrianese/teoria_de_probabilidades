...
Recordamos el Teorema de los Tiempos de Parada
...
\begin{enumerate}[ref=\textbf{\theexercise.}\textit{\labelenumi}]
	\item \(\Expectation(\martingale_S \mid \sEvents_R) = \martingale_R\) a.s. si \(R, S\) estan acotados. 
	\item \(\left( \martingale^S_t \right)_{t \geq 0}\) es una martingala.
	\item 
...
En el ejercicio \ref{exercise:4.1} probamos que \(\wiener_t = \brownian_t^2 - t\) es una martingala, y el teorema de los tiempos de parada deduce de esto que \(\wiener^T_t = \wiener_{t \wedge T}\) también es una martingala.
...
\end{align}
Por el teorema de los valores intermedios que acompaña a la continuidad de las trayectorias del movimiento browniano, y la forma concreta del tiempo de parada \(T = \inf \{t \geq 0 : B_t \in \{-y, x\}\}\), podemos acotar el valor aleatorio \(\left| B_{t \wedge T} \right|\) por una constante determinista \(c = x + y\).
Luego
\begin{align}
	\left| \wiener^T_t \right|
	\leq
...
El tiempo de parada \(T\) es integrable porque \(\Expectation T = \int_0^{\infty} \prob(T > t) \dd t\) y este integrando puede estudiarse.
En efecto

Por el teorema de los valores intermedios que acompaña a la continuidad de las trayectorias del movimiento browniano, y la forma concreta del tiempo de parada \(T = \inf \{t \geq 0 : B_t \in \{-y, x\}\}\),
\begin{align}
	\prob(T > t)
	=
...
El evento \(\left[ - y < B_s < x \;(\forall s \in [0, t]) \right]\), en un principio complejo al tratarse de una intersección nonumerable, puede simplificarse si consideramos sólo los valores enteros de \(s\).
Es decir, que este evento complejo esta contenido en otro \(\left[ - y < B_i < x \;(\forall i \in \{0, 1, \dots, \floor{t}\}) \right]\) expresable usando finitas intersecciones.
Además, para cada \(i \in \{1, \dots, \floor{t}\}\), el par de desigualdades \(- y < B_{i - 1}, B_i < x\) implica \(\abs{B_i - B_{i - 1}} \leq x + y = c\).
Luego
\begin{align}
	\prob&(- y < B_s < x \;(\forall s \in [0, t]))
	\\
	&\leq
	\prob(- y < B_i < x \;(\forall i \in \{0, 1, \dots, \floor{t}\}))
	\\
	&=
	\prob(- y < B_{i - 1}, B_i < x \;(\forall i \in \{1, \dots, \floor{t}\}))
	\\
	&\leq
	\prob(\abs{B_i - B_{i - 1}} \leq c \;(\forall i \in \{1, \dots, \floor{t}\}))
\end{align}

Por definición de MBE, los incrementos \(B_i - B_{i - 1}\) son mutuamente independientes entre sí, y tienen distribución \(\normal(0, 1)\).
Luego existe una constante \(\rho \in ]0, 1[\) tal que \(\rho = \prob(\abs{B_i - B_{i - 1}} \leq c)\) para todo \(i\), y
\begin{align}
	\prob(\abs{B_i - B_{i - 1}} \leq c \;(\forall i \in \{1, \dots, \floor{t}\}))
	\leq
	\rho^{\floor{t}}
\end{align}
En fin, la cota que conseguimos es
\begin{align}
