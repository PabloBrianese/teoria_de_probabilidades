...
\begin{exercise}
Sea $\brownianProcess$ un Movimiento Browniano estándar en dimensión 1.
...
\begin{enumerate}[label=\alph*)]
	\item \label{exercise:3.a)} Probar que \(\prob(T_x < + \infty) = 1\) para todo \(x \in \realnum\).
	(Sugerencia: puede ser útil observar el \(\sup_{t \geq 0} \brownian_t\) y el \(\inf_{t \geq 0} \brownian_t\))
	\item Probar que \(\prob(T_x \leq t) = \prob(\abs{\brownian_t} \geq \abs{x}) = 2 - 2 F_{\brownian_1} \left( \abs{x} / \sqrt{t} \right)\) para todo \(x \in \realnum\) y para todo \(t \geq 0\).
...
Por definición, un movimiento browniano tiene trayectorias continuas.
Además, la 
	% Notas Movimiento Browniano.pdf %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{property}[5]
Sea \(\brownianProcess\) el Movimiento Browniano Estándar en \(\realnum\).
Sea \(S^* = \sup_{t \geq 0} \brownian_s\), \(I^* = \inf_{t \geq 0} \brownian\).
Entonces \(S^* = + infty\) y \(I^* = - \infty\) a.s.
\end{property}
