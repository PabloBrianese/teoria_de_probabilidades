...
\begin{align}
	...
	\prob(\brownian_t \geq \abs{x}) + \prob(\brownian_t \geq \abs{x})
\end{align}
Juntas, ambas consecuencias de la invarianza por rotaciones de la familia de los MBE nos sugienren probar la igualdad
\begin{align}
	\prob(T_y \leq t)
	=
	2 \prob(B_ t \geq y)
...
% Notas Movimiento Browniano.pdf %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Tengo que cambiar este principio de reflexión por el otro, menos pulenta
...
\end{theorem}
En nuestro caso, probamos la hipótesis \(\prob(T_y < \infty) = 1\) en el ejercicio \ref{exercise:3.a)}.
Luego \(\reflectedBrownian_t = \brownian_t \characteristic(t \leq T_y) + (2 \brownian_{T_y} - \brownian_t) \characteristic(t > T_y)\) con \(t \geq 0\) es MBE.
Esta reflexión del proceso es relevante porque podemos probar 
\begin{align}
	[T_y \leq t] 
	= 
	[\brownian_t \geq y] \cup \left[ \reflectedBrownian_t > y \right]
\end{align}
...
\begin{align}
	[T_y \leq t]
	&=
	[T_y \leq t, B_t \geq y] \sqcup [T_y \leq t, B_t < y]
\end{align}
La continuidad de las trayectorias del MBE \(\brownianProcess\) hace que, al comenzar en \(\brownian_0 = 0\) y superar a \(y \geq 0\) al llegar a \(B_t \geq y\), necesariamente alcance el valor \(y\) en el intervalo de tiempo \([0, t]\).
Luego \([T_y \leq t, B_t \geq y] = [B_t \geq y]\).
